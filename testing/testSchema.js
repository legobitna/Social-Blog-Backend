...
const generateData = async () => {
	try {
		...
		const photos = await getRandomImages(userNum * otherNum); // must not exceed 30
		...
		for (let i = 0; i < userNum; i++) {
			for (let j = 0; j < otherNum; j++) {
				await Blog.create({
					...
					images: [photos[0].urls.small],
					author: users[i]._id,
				}).then(async (blog) => {
					console.log('Created blog:' + blog.title);
					blogs.push(blog);
					photos.shift();
					console.log(`| Each blog has ${otherNum} reviews from ${otherNum} random users`);
					console.log('-------------------------------------------');
					...
					console.log(`| Each blog has ${otherNum} reactions from ${otherNum} random users`);
					console.log('-------------------------------------------');
...
// min 1 max 30
const getRandomImages = async (num) => {
	let url = `https://api.unsplash.com/photos/random?count=${num}&orientation=squarish`;
