...
userController.findPassword = async (req, res, next) => {
  const { email } = req.body;
  console.log("reqbodybody", req.body);
  console.log("Eeeee", email);
  let user = await User.findOne({ email });

  if (!user) {
    return next(new AppError(409, "we dont have that email"));
  }
  ...
  return sendResponse(res, 200, true, { user }, null, "find password success");
};
